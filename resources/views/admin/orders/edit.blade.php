...
@section('breadcrumb')
	<ul class="breadcrumb">
	    ...
	    <li class="breadcrumb-item"><a href="javascript:">Edit Order</a></li>
	</ul>
@endsection
                  ...
                  <form method="post" action="{{ route('admin.orders.update', $order->id) }}">
                    ...
                    <div>
                      <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter Name" name="customer" value="{{ old('customer', $order->customer) }}">
                      </div>
                    </div>
                        <div class="table-responsive" x-data="alpine()" x-init="setRow()">
    ...
    <script type="text/javascript">
      function alpine() {
        ...
        return {
          ...
          newRow: @json($order->orderDetails),

          total: order.total,

          setRow(rows = @json($order->orderDetails)) {
            for (var i = 0; i < rows.length; i++) {
              let idet = rows[i].item_detail;

              const item = items.find(item => item.id == idet.item_id);
              ...
              rows[i].sizes = size;
              rows[i].variant_id = idet.variant_id;
              rows[i].size_id = idet.size_id;
              rows[i].price = idet.price;
              console.log(rows[i]);
            };
            this.rows = rows;
            for (var i = 0; i < rows.length; i++) {
              this.setItemDetail(i);
            };
